...
const expresiones = {
	...
	securityAnswer: /^[a-zA-Z0-9\_\-]{1,16}$/, // Letras, numeros, guion y guion_bajo
}
...
const validarFormulario = (e) => {
	switch (e.target.name) {
		case "firstName":
			validarCampo(expresiones.firstName, e.target, 'nombre');
		break;
        case "lastName":
		...
		break;
		case "securityAnswer":
			validarCampo(expresiones.securityAnswer, e.target, 'respuesta');
		break;
	}
}
...
formulario.addEventListener('submit', (e) => {
	...
	if(campos.nombre && campos.apellido && campos.password  && campos.telefono && campos.correo && campos.respuesta  && terminos.checked ){
		formulario.reset();
